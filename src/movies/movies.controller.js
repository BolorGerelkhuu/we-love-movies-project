...
async function readMoviesWithTheaters(req, res, next) {
  const data = await moviesService.listMoviesWithTheaters(
...
async function readMoviesWithReviews(req, res, next) {
  const data = await moviesService.listMoviesWithReviews(
    Number(req.params.movieId)
  );
  res.json({ data });
}
...
module.exports = {
  ...
  readMoviesWithTheaters: [
    asyncErrorBoundary(movieExists),
    asyncErrorBoundary(readMoviesWithTheaters),
  ],
  readMoviesWithReviews: [
    asyncErrorBoundary(movieExists),
    asyncErrorBoundary(readMoviesWithReviews),
  ],
};
